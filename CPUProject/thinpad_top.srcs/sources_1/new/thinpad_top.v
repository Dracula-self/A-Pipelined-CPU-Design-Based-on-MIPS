...
// 修正输出逻辑
assign ext_ram_be_n = (write_pending || (ext_ram_we_n_reg == 1'b0)) ? 
                      ext_ram_be_n_reg :  // 写操作时使用锁存的值
                      ((data_sram_en && is_data_ext_ram && data_sram_wen == 4'b0000) ? 
                       4'b0000 : 4'b1111); // 读操作时使用0000
...
always @(posedge clk_10M) begin
    ...
    // 添加流水线状态显示
    $display("Pipeline State at time %t:", $time);
    $display("IF Stage - PC: %h", cpu.pc_F);
    $display("ID Stage - Inst: %h", cpu.inst_D);
    $display("EX Stage - ALU result: %h", cpu.alu_out_E);
    $display("MEM Stage - Mem addr: %h, data: %h", cpu.alu_out_M, cpu.write_data_M);
    $display("WB Stage - Write reg: %d, data: %h", cpu.write_reg_W, cpu.result_W);
    $display("--------------------");
end
